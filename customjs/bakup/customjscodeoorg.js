...
function editOrganization(ths) 
{
  ...
  //console.log(rowValue);
   if(rowValue !== undefined)
    ...
    // else
    // {
    //   $('#country_id').val(editVal.country).prop('disabled',true).trigger('chosen:updated');
    //   $('#state_id').val(editVal.state_id).prop('disabled',true).trigger('chosen:updated');       
    // }
...
function editContact(ths) 
{
  ...
  //console.log(rowValue);
   if(rowValue !== undefined)
    ...
    // else
    // {
    //   $('#org_name').val(editVal.org_name).prop('disabled', true).trigger('chosen:updated');
    //   $('#state_id').val(editVal.state_id).prop('disabled', true).trigger('chosen:updated');
    //   $('#country_id').val(editVal.country).prop('disabled', true).trigger('chosen:updated');
    // }
    ...
    if(button_property != 'view')
     {
      ...
      $('#usr_role').val(editVal.role).prop('disabled', true).trigger('chosen:updated');
      $('#branch').val(editVal.brnh_id).prop('disabled', true).trigger('chosen:updated');
       if(editVal.brnh_usr != ''){
        chk_brnhuser(editVal.brnh_id,editVal.brnh_usr);
        //$('#brnh_usrs').css('display','block');
        //$('#usr_brnchid').val(editVal.brnh_usr).prop('selected', true);
        //$('#usr_brnchid').val(editVal.brnh_usr).prop('selected', true);
       }
     } 
...
//===========================Mutiple Organization Contact========================


$("#MultiOrgzCntForm").validate({
      rules: {
       multi_org: "required",
       org_details:"required"
      },
              ...
              //data : $('#OrganizationForm').serialize(), // serializes the form's elements.
                success : function (data) {
                  ...
                  if(data != ''){
                    ...
                    $("#resultmultiorg").text(msg); 
                    setTimeout(function() {   //calls click event after a certain time
                    $("#mutiOrgCntModal .close").click();
                    $("#resultmultiorg").text(" "); 
                    $('#MultiOrgzCntForm')[0].reset(); 
                    $("#morg_id").val("");
                    $("#morgload").css("display","none");
                    $("#morgsave").css("display","inline-block");
                    $("#morgsave").attr("type","submit");
...
function ajex_multiOrgCntData(thsid){
  ur = "ajax_ListMultiOrgCntData";
    $.ajax({
      url: ur,
      type: "POST",
      data:{'id':thsid},
