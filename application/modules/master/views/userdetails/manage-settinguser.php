...
<tr>
  ...
  <th><div style="width: 41px;">SL No.</div></th>
  <th>Name</th>
  <th>Profile</th>
  <th>Email</th>
  <th>Last Login</th>
  ...
  <th style="text-align: right;">Action</th>
    ...
    <tbody id="dataTable">
      ...
      <td>
        ...
        <?php 
        $oldids=$this->db->query("select * from tbl_software_log where slog_type='User' AND old_id='$dt->user_id' ");
        $oldNumRow=$oldids->num_rows();
        $newids=$this->db->query("select * from tbl_software_log where slog_type='User' AND new_id='$dt->user_id' ");
        $newNumRow=$newids->num_rows();
        $mkrid=$this->db->query("select * from tbl_software_log where maker_id='$dt->user_id' ");
